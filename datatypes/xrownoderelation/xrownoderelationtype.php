...
{
    const DATA_TYPE_STRING = "xrownoderelation";
    /*!
     Initializes with a string id and a description.
    */
    function xrowNodeRelationType()
    {
        $this->eZDataType( self::DATA_TYPE_STRING, ezpI18n::tr( 'extension/xrownoderelation/datatypes', "Node relation", 'Datatype name' ),
    ...
    */
    function validateObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        if ( $http->hasPostVariable( $postVariableName ) )
        {
            ...
            {
                $contentObjectAttribute->setValidationError( ezpI18n::tr( 'extension/xrownoderelation/datatypes',
                                                                     'Missing noderelation input.' ) );
                return eZInputValidator::STATE_INVALID;
            }
        }
        else if ( $contentObjectAttribute->validateIsRequired() )
        {
            $contentObjectAttribute->setValidationError( ezpI18n::tr( 'extension/xrownoderelation/datatypes', 'Missing noderelation input.' ) );
            return eZInputValidator::STATE_INVALID;
        }
        ...
        switch ( $action )
        {
            case "set_object_relation" :
            {
                if ( $http->hasPostVariable( 'BrowseActionName' ) and
                          $http->postVariable( 'BrowseActionName' ) == ( 'AddRelatedNode_' . $contentObjectAttribute->attribute( 'id' ) ) and
                          $http->hasPostVariable( "SelectedNodeIDArray" ) )
                {
                    if ( !$http->hasPostVariable( 'BrowseCancelButton' ) and $http->hasPostVariable( "SelectedNodeIDArray" ) )
                    {
                        ...
                        $contentObjectAttribute->setAttribute( 'data_int', $selectedNodeIDArray[0] );
            ...
            case "browse_object" :
            {
                ...
                $browseParameters = array( 'action_name' => 'AddRelatedNode_' . $contentObjectAttribute->attribute( 'id' ),
                                           'type' =>  'AddRelatedNodeToDataType',
            ...
            default :
            {
                eZDebug::writeError( "Unknown custom HTTP action: " . $action, "xrownoderelation" );
            } break;
        }
    }
    ...
    */
    ...
    {
        $NodeID = $contentObjectAttribute->attribute( "data_int" );
        if ( $NodeID != 0 )
...
eZDataType::register( xrowNodeRelationType::DATA_TYPE_STRING, "xrowNodeRelationType" );
